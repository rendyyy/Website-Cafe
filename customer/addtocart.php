        ...
        $nomor = date("YmdHis");
				...
				if ($stock == 0) {
		          echo "<script>alert('Stock Habis, silahkan pilih produk lain!'); window.location = 'javascript:history.back()'</script>";
						...
						$insert = mysqli_query($koneksi, "INSERT INTO cart(id_cart, tanggal, kode, nama, harga, qty, jumlah, kd_cus)
						                                  VALUES('$nomor', '$tanggal', '$kode', '$nama', '$harga', '$qty', '$jumlah', '$session');") or die(mysqli_error($query));
