...
<body>
	...
	<div id="page-title">
		...
		<div id="page-title-inner">
			...
			<div class="container">
				...
				<h2><i class="ico-shopping-cart ico-white"></i>Cart</h2>
 ...
 <?php
             if(isset($_GET['aksi']) == 'delete'){
				$id = $_GET['kd'];
				$cek = mysqli_query($koneksi, "SELECT * FROM cart WHERE kode='$id' AND kd_cus='$_SESSION[user_id]'");
                $data = mysqli_fetch_array($cek);
                $moq = $data['qty'];
				if(mysqli_num_rows($cek) == 0){
					echo '<div class="alert alert-success alert-dismissable"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button> Data tidak ditemukan.</div>';
				}else{
					$delete = mysqli_query($koneksi, "DELETE FROM cart WHERE kode='$id' AND kd_cus='$_SESSION[user_id]'");
					if($delete){
 ...
 <?php
             if(isset($_GET['add']) == 'add'){
				$id_add = $_GET['kd1'];
                $qty = 1;
				$cek1 = mysqli_query($koneksi, "SELECT * FROM cart WHERE kode='$id_add' AND kd_cus='$_SESSION[user_id]'");
                ...
                if(mysqli_num_rows($cek1) == 0){
					...
					$add = mysqli_query($koneksi, "UPDATE cart SET qty=(qty+'$qty'), jumlah='$jum' WHERE kode='$id_add' AND kd_cus='$_SESSION[user_id]'");
					if($add){
 ...
 <?php
             if(isset($_GET['less']) == 'less'){
				$id_less = $_GET['kd2'];
                $qty1 = 1;
				$cek2 = mysqli_query($koneksi, "SELECT * FROM cart WHERE kode='$id_less' AND kd_cus='$_SESSION[user_id]'");
                ...
                if(mysqli_num_rows($cek2) == 0){
					...
					$less = mysqli_query($koneksi, "UPDATE cart SET qty=(qty-'$qty1'), jumlah='$jum1' WHERE kode='$id_less' AND kd_cus='$_SESSION[user_id]'");
					if($less){
...
<tr>
            ...
            ?>
				...
				if($total <= 10000){
					...
					echo '
						<tr style="background-color: #DDD;"><td colspan="6" align="right"><b>Ongkos Kirim :</b></td><td align="right"><b>Rp. 10.000,00</b></td></td></td><td></td></tr>
						...
						<a href="bayar.php" class="btn"><i class="glyphicon glyphicon-shopping-cart icon-white"></i> CHECK OUT &raquo;</a>
						</div></p> 
					';
				}
